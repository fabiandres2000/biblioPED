                                                                    ...
                                                                    <div class="form-card">
                                                                        ...
                                                                        <div class="row">
                                                                            <div class="col-lg-3" v-for="(item, index) in asignaturas_filtradas" :key="index" style="margin-bottom: 20px">
                                                                                <div  :id="'ra'+index" :class="asignatura == item.nombre ? 'card_radio_active' : 'card_radio'" @click="select_radio_2('ra'+index, 'carda'+index)">
                                                                                    <input class="radio_b" :value="item.nombre" v-model="asignatura" type="radio" name="asignatura" :id="'carda'+index">
                                                                ...
                                                                <fieldset>
                                                                    <div class="form-card">
                                                                        ...
                                                                        <div class="row" style="margin-top: 30px">
                                                                            <div class="col-12">
                                                                                <h2 class="fs-title" v-if="tipo_contenido=='normal'">Ingresa el enunciado</h2>
                                                                                <h2 class="fs-title" v-if="tipo_contenido=='metafacto'">Selecciona la imagen que desea cargar</h2>
                                                                                <h2 class="fs-title" v-if="tipo_contenido=='video'">Ingresa el contenido y selecciona el video que desea cargar</h2>
                                                                            </div>
                                                                        </div>
                                                                        <br>
                                                                        <div v-if="tipo_contenido=='normal' || tipo_contenido=='video'">
                                                                            <QuillEditor v-model:content="contenido_html" contentType="html" style="height: 300px;" theme="snow" toolbar="full" />
                                                                            <br>
                                                                            <br>
                                                                        </div>
                                                                    ...
                                                                    <br>
                                                                    <button  name="previous" type="button" class="previous_btn btn btn-secondary mr-sm-1 mb-1 mb-sm-0"><i class="fas fa-arrow-left"></i> Anterior</button>
                                                                    <button v-if="editable" name="next" @click.prevent="guardarDatosContenido" type="button" class="save btn btn-primary mr-sm-1 mb-1 mb-sm-0">Guardar <i class="fas fa-save"></i></button>
                                                                    <button style="width: 150px !important" v-if="!editable" name="next" @click.prevent="goBack" type="button" class="save btn btn-warning mr-sm-1 mb-1 mb-sm-0">Volver a la lista <i class="fas fa-undo"></i></button>
                                                                </fieldset>
                                                            </form>
                                                        </div>
                                                    </div>
...
export default {
    ...
    data(){
        return {
            ...
            tipo_subida: '',
            tipo_contenido: '',
            tipo_subida_anterior: '',
            tipo_contenido_anterior: '',
            asignatura: '',
            grado: '',
            tema: '',
            imageFile: null,
    ...
    methods: {
        mostrarObjeto() {
            const objetoSerializado = this.$route.params.objeto;
            this.editable = JSON.parse(this.$route.params.editable);
            const objetoDeserializado = JSON.parse(objetoSerializado);
            
            const radioAVerificar = document.querySelector('input[value="'+objetoDeserializado.tipo_contenido+'"]');
            radioAVerificar.checked = true;
            radioAVerificar.parentElement.classList.remove('card_radio');
            radioAVerificar.parentElement.classList.add('card_radio_active');

            this.tipo_subida = objetoDeserializado.tipo_contenido;
            this.tipo_subida_anterior = objetoDeserializado.tipo_contenido;
            ...
            const radioAVerificar2 = document.querySelector('input[value="'+objetoDeserializado.tipo_multimedia+'"]');
            radioAVerificar2.checked = true;
            radioAVerificar2.parentElement.classList.remove('card_radio');
            radioAVerificar2.parentElement.classList.add('card_radio_active');

            this.tipo_contenido = objetoDeserializado.tipo_multimedia;
            this.tipo_contenido_anterior = objetoDeserializado.tipo_multimedia
            this.asignatura = objetoDeserializado.asignatura;
            this.grado = objetoDeserializado.grado;
            

            if(this.tipo_contenido == "video" || this.tipo_contenido == "normal"){
                this.contenido_html  = objetoDeserializado.cont_documento;
            }

            if(this.tipo_contenido == "normal" ){
                this.tema = objetoDeserializado.titulo;
            }

            if(this.tipo_contenido == "video" ){
                this.selectedVideo = "/videos/"+objetoDeserializado.ruta;
                this.tema = objetoDeserializado.tema;

                const rutaArchivo = objetoDeserializado.ruta;

                fetch('/videos/' + rutaArchivo)
                .then(response => response.blob())
                .then(blob => {
                    const archivo = new File([blob], objetoDeserializado.ruta, { type: 'video/mp4' });
                    return archivo;
                })
                .then(archivo => {
            ...
            if(this.tipo_contenido == "metafacto" ){
                this.selectedImage = "/metafactos/"+objetoDeserializado.ruta;
                this.tema = objetoDeserializado.tema;

                const rutaArchivo = objetoDeserializado.ruta;

                fetch('/metafactos/' + rutaArchivo)
                .then(response => response.blob())
                .then(blob => {
                    const archivo = new File([blob], objetoDeserializado.ruta, { type: 'image/jpg' });
                    return archivo;
                })
                .then(archivo => {
                    this.imageFile = archivo;
                })
                .catch(error => {
                    console.error(error);
                });
            ...
            this.oid = objetoDeserializado._id.$oid;
        },
        goBack() {
            this.$router.go(-1);
        },
        ...
        async guardarDatosContenido() {
            ...
            formData.append('tipo_subida_anterior', this.tipo_subida_anterior);
            formData.append('tipo_contenido_anterior', this.tipo_contenido_anterior);
            ...
            if(this.tipo_contenido != "" && this.tipo_subida != "" && this.grado != "" && this.tema != "" && this.asignatura != ""){
                ...
                if(!bandera){
                    this.loading = true;
                    try {
                        const response = await adminService.editarDatosContenido(formData);
                        ...
                        toastr.error('Error al modificar los datos'+ error);
                        this.loading = false;
                    }
                }else{
